...
class Kadın extends Command {
       ...
       Schema.findOne({sunucu: message.guild.id, user: member.id}, async(err, dbres) => {
        if(!dbres) {
            ...
            new Schema({sunucu: message.guild.id, user: member.id, isimler: arr}).save().catch(err => console.log(`isimler (erkek.js) kayıt edilemedi\nSorun:`+ err))
                ...
                .setDescription(`${member} üyesine ${stg.Roles.womanRoles.map(x => `<@&${x}>`)} rolleri verildi.`)
                .setColor('RANDOM')
            ...
            let embed1 = new Discord.MessageEmbed()
            .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
            .setDescription(`${member} üyesine ${stg.Roles.womanRoles.map(x => `<@&${x}>`)} rolleri verildi, bu üye daha önce farklı isimlerle kayıt olmuş.
    ...
    setTimeout(() => { member.roles.add(stg.Roles.womanRoles).catch(err => console.log(`kadın rolü verilemedi\nsorun:`+ err)) }, 1500)
