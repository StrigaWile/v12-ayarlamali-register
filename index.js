...
class Striga extends Client {
    constructor(options) {
        super(options);
       this.config = require('./config')
       this.registerTimer = new Map();
   ...
   loadCommand(commandPath, commandName) {
    try {
      ...
      return false;} catch (e) {return `Komut yüklenirken hata oluştu: ${commandName}: ${e}`;
    }
  }
...
async turkishDate(date) {
  ...
  async kalıcı(mesaj, msg, kanal) {
    if (!mesaj || typeof mesaj !== "string") return
    const embd = new MessageEmbed().setAuthor(msg.tag, msg.displayAvatarURL({ dynamic: true })).setColor("RANDOM").setDescription(mesaj)
    kanal.send(embd)}

    async cezatext(mesaj, msg, kanal) {
      if (!mesaj || typeof mesaj !== "string") return
      const embd = new MessageEmbed().setAuthor(msg.tag, msg.displayAvatarURL({ dynamic: true })).setColor("RANDOM").setDescription(mesaj)
      kanal.send(embd).then(msg => {msg.delete({ timeout: 25000 }).catch(err => {{}})}).catch(err => {{}});}

  async uzun(mesaj, msg, kanal) {
    if (!mesaj || typeof mesaj !== "string") return
    const embd = new MessageEmbed().setAuthor(msg.tag, msg.displayAvatarURL({ dynamic: true })).setColor("RANDOM").setDescription(mesaj)
    kanal.send(embd).then(msg => {msg.delete({ timeout: 35000 }).catch(err => {{}})}).catch(err => {{}});}

    async normal(mesaj, msg, kanal) {
        if (!mesaj || typeof mesaj !== "string") return
        const embd = new MessageEmbed().setAuthor(msg.tag, msg.displayAvatarURL({ dynamic: true })).setColor("RANDOM").setDescription(mesaj)
        kanal.send(embd).then(msg => {msg.delete({ timeout: 15000 }).catch(err => {{}})}).catch(err => {{}});}

        async hızlı(mesaj, msg, kanal) {
            if (!mesaj || typeof mesaj !== "string") return
            const embd = new MessageEmbed().setAuthor(msg.tag, msg.displayAvatarURL({ dynamic: true })).setColor("RANDOM").setDescription(mesaj)
            kanal.send(embd).then(msg => {msg.delete({ timeout: 7500 }).catch(err => {{}})}).catch(err => {{}});}
            

}// CLASS 


const client = new Striga();
...
const evtFiles = await readdir("./Events/");
  console.log(`Toplam ${evtFiles.length} event yükleniyor.`);
...
mongoose.connect(client.config.MongoURL, { useNewUrlParser: true, useUnifiedTopology: true }).then(console.log(`Client ile Mongo bağlantısı sağlandı. ✔`))
client.login(client.config.Client_Token).then(console.log(`Client ile bağlantı kuruldu. ✔`)).catch(console.error)
